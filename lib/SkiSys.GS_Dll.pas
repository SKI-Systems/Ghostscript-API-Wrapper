...
{$ELSE} //Delphi
  {$DEFINE DELPHI}
  {$IFDEF MSWINDOWS}
    // remome warning platform 'DELAYED'
...
{$ENDIF}
  ...
  // FPC will do this even without delayed
...
{$ENDIF}
  ...
  function gsapi_revision(pr: p_gsapi_revision_t; len: Integer): Integer;
                          ...
                          external GS_DLL
                          {$IFDEF DELPHI}{$IFDEF MSWINDOWS}delayed{$ENDIF}{$ENDIF};
  ...
  function gsapi_run_string_continue(instance: Pointer;
                                     const str: PAnsiChar; length: Cardinal;
                                     user_errors: Integer;
                                     out pexit_code: Integer): Integer;
                                     {$IFDEF MSWINDOWS}stdcall;{$ELSE}cdecl;{$ENDIF}
                                     external GS_DLL
